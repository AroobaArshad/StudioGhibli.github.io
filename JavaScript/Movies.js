...
document.addEventListener('DOMContentLoaded', function() {
    const score1Elements = document.querySelectorAll('.score1');
    const score2Elements = document.querySelectorAll('.score2');
    ...
    function handleIntersection(entries, observer) {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                // Once the element is in the viewport, disconnect the observer to improve performance
                observer.unobserve(entry.target);
            }
        });
    }

    // Create intersection observers for score1 elements
    const score1Observer = new IntersectionObserver(handleIntersection, {
        root: null,
        rootMargin: '0px',
        threshold: 0.2 // Adjust this threshold as needed
    });

    // Observe each score1 element
    score1Elements.forEach(score1 => {
        score1Observer.observe(score1);
    });

    // Create intersection observers for score2 elements
    const score2Observer = new IntersectionObserver(handleIntersection, {
        root: null,
        rootMargin: '0px',
        threshold: 0.2 // Adjust this threshold as needed
    });

    // Observe each score2 element
    score2Elements.forEach(score2 => {
        score2Observer.observe(score2);
    });
});